...
testutils.testWithHeader('run-cmake must skip build when generation fails with cmakesettings.json file', async () => {
  const answers: testutils.BaseLibAnswers = {
    "exec": {
      [`${gitPath}`]:
        { code: 0, stdout: "git output" },
      [`${cmakeExePath} -GVisual Studio 16 2019 -Ax64 -DCMAKE_TOOLCHAIN_FILE:string=D:/src/vcpkg/scripts/buildsystems/vcpkg.cmake -DCMAKE_MSBUILD_VS_FLAG:boolean=ON ${path.dirname(cmakeSettingsJsonPath)}`]: { 'code': 1, "stdout": 'cmake generation failure output here' },
      [`${cmakeExePath} -GNinja -DCMAKE_MAKE_PROGRAM=${ninjaExePath} -DCMAKE_BUILD_TYPE=Debug -DCMAKE_ANY_FLAG:boolean=OFF -DCONFIGURATION:STRING=ResolvedConfiguration ${path.dirname(cmakeSettingsJsonPath)}`]: { 'code': 1, "stdout": 'cmake generation failure output here' },
  ...
  expect(cmakeBuildMock).toBeCalledTimes(0);
