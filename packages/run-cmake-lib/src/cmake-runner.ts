  ...
  private handleVcpkgJsonManifestExistence(cmakeSourceDir: string, cmakeBuildDir: string): void {
    if (fs.existsSync(cmakeSourceDir)) {
      ...
      if (fs.existsSync(vcpkgJsonLocation)) {
        ...
        this.tl.info(`Found vcpkg manifest at ${vcpkgJsonLocation}, adding to run-vcpkg cached path '${additionalPath}'.`);
        runvcpkg.setAdditionalCachedPaths(this.tl, additionalPath);
      }
    }
  }
}
