...
jest.spyOn(utils.BaseLibUtils.prototype, 'readFile').mockImplementation(
  function (this: utils.BaseLibUtils, file: string): [boolean, string] {
    if (testutils.areEqualVerbose(file, path.join(vcpkgRoot, '.artifactignore'))) {
      ...
      return [true, vcpkgGitRef];
    }
    else if (testutils.areEqualVerbose(file, path.join(vcpkgRoot, responseFile))) {
...
mock.inputsMocks.setInput(globals.vcpkgArguments, 'vcpkg_args @respfile --overlay-ports=inlineport');
mock.inputsMocks.setInput(globals.vcpkgTriplet, vcpkgTriplet);
mock.inputsMocks.setInput(globals.vcpkgCommitId, vcpkgGitRef);
...
testutils.testWithHeader('run-vcpkg with overlays must build and install successfully', async () => {
  const answers: testutils.TaskLibAnswers = {
    "exec": {
      ...
      [`${gitPath} checkout --force ${vcpkgGitRef}`]:
        { 'code': 0, 'stdout': `this is git checkout ${vcpkgGitRef} output` },
