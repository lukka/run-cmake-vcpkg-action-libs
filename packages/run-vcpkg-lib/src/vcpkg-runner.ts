...
export class VcpkgRunner {
  ...
  private static readonly vcpkgInstallCmdDefault: string = `install --recurse --clean-after-build --vcpkg-root=\"\${{${globals.vcpkgDirectory}}}\"`;

  public static async run(
    baselib: baselib.BaseLib,
    vcpkgInstallCmd: string = VcpkgRunner.vcpkgInstallCmdDefault): Promise<void> {
    await using(baseutillib.Matcher.createMatcher('all', baselib, process.cwd()),
      async () => {
        const vcpkgRunner: VcpkgRunner = new VcpkgRunner(baselib, vcpkgInstallCmd);
        await vcpkgRunner.run();
      });
  }

  public constructor(private tl: baselib.BaseLib, private vcpkgInstallCmd: string = VcpkgRunner.vcpkgInstallCmdDefault) {
    ...
    this.doRunVcpkgInstall = this.tl.getBoolInput(globals.doRunVcpkg, false) ?? false;
