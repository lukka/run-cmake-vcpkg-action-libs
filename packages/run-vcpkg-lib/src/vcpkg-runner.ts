...
export class VcpkgRunner {
  ...
  public static async run(baselib: baselib.BaseLib): Promise<void> {
    const vcpkgRunner: VcpkgRunner = await VcpkgRunner.create(baselib);
  ...
  public async run(): Promise<void> {
    await using(baseutillib.Matcher.createMatcher('all', this.baseUtils.baseLib, process.cwd()),
      async () => this.runImpl());
  }
